...
% path_to_feature
% mat_path = '/DATACENTER/3/qxl/demo_viper/feature/';
mat_path = '../../feature/';
%% calculate query features
%Hist_query = importdata([mat_path, 'Hist_query.mat']);
%nQuery = size(Hist_query, 2);
nQuery = 316; 

%% calculate database features
%Hist_test = importdata([mat_path, 'Hist_test.mat']);
...
if ~exist([mat_path, 'testID.mat'])
    ...
    save([mat_path, 'testID.mat'], 'testID');
    save([mat_path, 'testCAM.mat'], 'testCAM');
else
    testID = importdata([mat_path,'testID.mat'])';
    testCAM = importdata([mat_path,'testCAM.mat'])';    
end
...
if ~exist([mat_path,'queryID.mat'])
    ...
    save([mat_path,'queryID.mat'], 'queryID');
    save([mat_path,'queryCAM.mat'], 'queryCAM');
else
    queryID = importdata([mat_path,'queryID.mat'])';
    queryCAM = importdata([mat_path,'queryCAM.mat'])';    
end
...
dist = importdata([mat_path, 'dist_concat.mat']);
...
CMC = mean(CMC);
%% print result
fprintf('single query:                                    mAP = %f, r1,5,10 precision = %f,%f,%f\r\n', mean(ap), CMC(1),CMC(5),CMC(10));
